...
/*Classes*/
$table = new TableSupplier($pagination, 'activities', 'bedrijf', $id);
$activities = new Activities('', $table->limit);
$loginValidate = new validateLogin();
$loginValidate->securityCheck();
$search = $activities->findAllId($table->startfrom, $table->limit, $id, 3);
...
<div class="card-header border-0 bg-white rounded-top p-4">
    <div class="row px-4 align-items-center justify-content-between">
        ...
        <a class="btn btn-info rounded text-white"
           href="/taken/nieuw?relation=<?php echo urlencode($supplier->getData('name')); ?>&id=<?php echo $user->data['id'];?>">Nieuwe taak</a>
        ...
        $.ajax({ //Process the form using $.ajax()
            ...
            success: function (data) {
                ...
                if (data.status === 'success') {
                    ...
                    $('#supplier_tasks').load('/controllers/tables/supplier/supplier_activities.php?page=' + page + '&id=' + id, function () {
                    });
                } else {
